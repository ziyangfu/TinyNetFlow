...
//
#include "netflow/OSAdaptor/include/IO/net/InetAddr.h"
...
BOOST_AUTO_TEST_CASE(testInet6Address)
{
    ...
    /** 虽然ipv6此时为false，但是ip地址中有：，因此使用strchr函数判断，等同与ipv6= true */
...
BOOST_AUTO_TEST_CASE(testInetAddressResolve)
{
    ...
    {
        STREAM_INFO << "google.com resolved to " << addr.toIpPort();
    }
    else
    {
        STREAM_ERROR << "Unable to resolve google.com";
    }
}
