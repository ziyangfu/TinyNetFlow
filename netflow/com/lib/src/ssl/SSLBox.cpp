/*!
 * \brief openSSL封装
 * 改写自开源项目：ZLToolKit(https://github.com/ZLMediaKit/ZLToolKit)
 * */

#include "ssl/SSLBox.h"
...
int SSL_Initor::findCertificate(SSL *ssl, int *, void *arg) {
    ...
    if (vhost && vhost[0] != '\0') {
        ...
        if (!ctx) {
            //未找到对应的证书
            SPDLOG_WARN("Can not find any certificate of host: {}, select default certificate of: {}",
                        vhost, ref._default_vhost[(bool) (arg)]);
        }
    }
    ...
    if (!ctx) {
        //未有任何有效的证书
        SPDLOG_WARN("Can not find any certificate of host: {}, tls handshake failed",
                    (vhost ? vhost : "default host"));
        return SSL_TLSEXT_ERR_ALERT_FATAL;
    }
...
bool SSL_Initor::setContext(const string &vhost, const shared_ptr<SSL_CTX> &ctx, bool server_mode, bool is_default) {
    ...
    if (vhost.empty()) {
        ...
        SPDLOG_DEBUG("Add certificate of: {}", vhost);
    }
    return true;
#else
    SPDLOG_WARN("ENABLE_OPENSSL disabled, you can not use any features based on openssl");
    return false;
#endif //defined(ENABLE_OPENSSL)
}
...
void SSL_Initor::setupCtx(SSL_CTX *ctx) {
    ...
    SSL_CTX_set_verify(ctx, SSL_VERIFY_NONE, [](int ok, X509_STORE_CTX *pStore) {
        if (!ok) {
            ...
            SPDLOG_WARN("SSL_CTX_set_verify callback, depth: {} ,err: {}",
...
std::shared_ptr<SSL_CTX> SSL_Initor::getSSLCtx_l(const string &vhost_in, bool server_mode) {
    ...
    if (vhost.empty()) {
        if (!_default_vhost[server_mode].empty()) {
            ...
            //没默认主机，选择空主机
            if (server_mode) {
                SPDLOG_WARN("Server with ssl must have certification and key");
            }
...
void SSL_Box::shutdown() {
    ...
    if (ret != 1) {
        SPDLOG_WARN("SSL_shutdown failed:{}", SSLUtil::getLastError());
    } else {
        flush();
    }
#endif //defined(ENABLE_OPENSSL)
...
/*!
 * \brief 将接收到的数据，放入ssl的接收缓冲区，并尝试解密
 * */
void SSL_Box::onRecv(const std::shared_ptr<Buffer> &buffer) {
    ...
    while (buffer->readableBytes()) {
        auto nwrite = BIO_write(_read_bio, buffer->peek(),
                                static_cast<int>(buffer->readableBytes()));
        if (nwrite > 0) {
            buffer->retrieve(static_cast<size_t>(nwrite));
            flush();
        }
        else {
            //nwrite <= 0,出现异常
            SPDLOG_ERROR("Ssl error on BIO_write: {}", SSLUtil::getLastError());
            shutdown();
            break;
        }
    }
...
void SSL_Box::onSend(std::shared_ptr<Buffer> buffer) {
    if (!buffer->readableBytes()) {
        return;
    }
    if (!_ssl) {
...
void SSL_Box::setOnDecData(const function<void(const std::shared_ptr<Buffer> &)> &cb) {
    _on_dec = cb;
}

void SSL_Box::setOnEncData(const function<void(const std::shared_ptr<Buffer> &)> &cb) {
    _on_enc = cb;
}
...
//    });
    ...
    while (!_buffer_send.empty()) {
        ...
        while (front->readableBytes()) {
            auto nwrite = SSL_write(_ssl.get(), front->peek(),
                                    static_cast<int>(front->readableBytes()));
            if (nwrite > 0) {
                front->retrieve(static_cast<size_t>(nwrite));
                flushWriteBio();
            }
            else {
               break;
...
//        while (offset < front->readableBytes()) {
...
//        }
//
//        if (offset != front->readableBytes()) {
//            //这个包未消费完毕，出现了异常,清空数据并断开ssl
//            SPDLOG_ERROR("Ssl error on SSL_write: {}", SSLUtil::getLastError());
//            shutdown();
//            break;
//        }
