/*!
 * \brief openSSL黑盒封装，屏蔽了SSL的握手细节，支持多线程
 * 改写自开源项目：ZLToolKit(https://github.com/ZLMediaKit/ZLToolKit)
 * */

#ifndef COM_LIB_SSL_SSL_BOX_H
#define COM_LIB_SSL_SSL_BOX_H
...
public:
     ...
     */
    bool loadCertificate(const std::string &pem_or_p12,
...
private:
    ...
    bool setContext(const std::string &vhost,
                    const std::shared_ptr<SSL_CTX> &ctx,
...
public:
     ...
     */
    void onSend(std::shared_ptr<Buffer> buffer);

    /**
     * 设置解密后获取明文的回调
     * @param cb 回调对象
     */
    void setOnDecData(const std::function<void(const std::shared_ptr<Buffer> &)> &cb);

    /**
     * 设置加密后获取密文的回调
     * @param cb 回调对象
     */
    void setOnEncData(const std::function<void(const std::shared_ptr<Buffer> &)> &cb);
...
private:
    ...
    std::list<std::shared_ptr<Buffer>> _buffer_send;

    //ResourcePool <BufferRaw> _buffer_pool;
    std::function<void(const std::shared_ptr<Buffer> &)> _on_dec;
    std::function<void(const std::shared_ptr<Buffer> &)> _on_enc;
};

} /* namespace com */

#endif /* COM_LIB_SSL_SSL_BOX_H */
